    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            if (empty($errors)) {
                ...
                $stmt = $db->prepare("SELECT id, username, password, is_active FROM users WHERE email = ?");
                $stmt->execute([$email]);
                $user = $stmt->fetch(PDO::FETCH_ASSOC);

                if ($user) {
                    if (!$user['is_active']) {
                        $errors[] = 'Ваш аккаунт деактивирован. Пожалуйста, свяжитесь с администратором.';
                    } elseif (password_verify($password, $user['password'])) {
                        // Успешная авторизация
                        initUserSession($user['id'], $user['username']);
            ...
            logError("Ошибка при входе", ['email' => $email, 'error' => $e->getMessage()]);
            $errors[] = 'Ошибка при входе. Пожалуйста, попробуйте позже.';
        }
    }
}
...
<head>
    ...
    <title>Вход - Young Forever</title>
